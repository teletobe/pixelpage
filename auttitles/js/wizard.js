  ...
  },
  {
    ...
    options: [
      { label: 'Bakkalaureat — die solide Grundlage ist gelegt',
        next: 'q4_extra', data: { degreeLevel: 'bachelor', degreeField: 'tech' } },
      { label: 'Master oder Diplom — zumindest auf dem Papier fertig',
        next: 'q4_extra', data: { degreeLevel: 'master', degreeField: 'tech' } },
      { label: 'Doktorat — Großvater hat es in der Weihnachtskarte erwähnt',
        next: 'q4_extra', data: { degreeLevel: 'doctorate', degreeField: 'tech' } },
      { label: 'Habilitation — er versteht es nicht mehr ganz, findet es aber sehr eindrucksvoll',
        next: 'q4_extra', data: { degreeLevel: 'habilitation' } }
    ]
  },
  {
    id: 'q3_naja',
    ...
    options: [
      { label: 'Jus — das versteht er dann doch noch',
        next: 'q3_level', data: { degreeField: 'law' } },
      { label: 'Medizin — er ist sofort stolz, aber auch besorgt',
        next: 'q4_extra', data: { degreeLevel: 'master', degreeField: 'med' } },
      { label: 'Wirtschaft — er findet es irgendwie kapitalistisch, akzeptiert es aber',
        next: 'q3_level', data: { degreeField: 'other' } },
      { label: 'Geisteswiss., Philosophie, Kunst, Kommunikation...',
        next: 'q3_level', data: { degreeField: 'arts' } }
    ]
  },
  {
    id: 'q3_level',
    ...
    options: [
      { label: 'Bakkalaureat — ein ehrlicher Anfang',
        next: 'q4_extra', data: { degreeLevel: 'bachelor' } },
      { label: 'Master oder Magister — fertig, zumindest offiziell',
        next: 'q4_extra', data: { degreeLevel: 'master' } },
      { label: 'Doktorat — vom eigenen Fach tief überzeugt',
        next: 'q4_extra', data: { degreeLevel: 'doctorate' } }
    ]
  },
  {
    id: 'q4_extra',
    ...
    options: [
      { label: 'MBA — für den Schwenk in die Privatwirtschaft',
        next: 'q5', data: { _extra: 'mba' } },
      { label: 'Zweites Doktorat (DDr.) — aus Überzeugung oder Gewohnheit',
        next: 'q5', data: { _extra: 'dr2' } },
      { label: 'Ingenieur-Titel (Ing.) der Kammer — nicht wie DI, aber man trägt ihn',
        next: 'q5', data: { _extra: 'ing' } },
      { label: 'Nein, das genügt vollkommen',
        next: 'q5' }
    ]
  },
  // ── MILITÄR ──────────────────────────────────────────
  ...
  },
  {
    ...
    options: [
      { label: 'Unteroffizier (Vizeleutnant) — Respekt auf der Kaserne',
        next: 'q7', data: { militaryRank: 'vzlt' } },
      { label: 'Offizier (Hauptmann bis Major) — man trägt Verantwortung',
        next: 'q7', data: { militaryRank: 'hauptmann' } },
      { label: 'Stabsoffizier (Oberstleutnant–Oberst) — Logistik ist eine Leidenschaft',
        next: 'q7', data: { militaryRank: 'oberst' } },
      { label: 'Generalsrang (Brigadier oder höher) — die Epauletten sind echt',
        next: 'q7', data: { militaryRank: 'brigadier' } }
    ]
  },
  // ── BEAMTE ───────────────────────────────────────────
  {
    ...
    options: [
      { label: 'Ja, und ich habe es nie bereut',
        next: 'q8' },
  ...
  },
  {
    ...
    options: [
      { label: 'Amtsdirektor — bescheiden, aber ehrenwert',
        next: 'q9', data: { civilRank: 'amtsdirektor' } },
      { label: 'Hofrat — der Klassiker; klingt nach mehr, als man erwartet',
        next: 'q9', data: { civilRank: 'hofrat' } },
      { label: 'Ministerialrat — ein Wort, das Türen öffnet',
        next: 'q9', data: { civilRank: 'ministerialrat' } },
      { label: 'Sektionschef — hier endet die Karriereleiter. Und das Formular.',
        next: 'q9', data: { civilRank: 'sektionschef' } }
    ]
  },
  // ── EHRUNGEN ─────────────────────────────────────────
  {
  ...
  result.innerHTML = `
    ...
    <div class="result-breakdown">
      ...
      ${explanations.length > 0
        ? explanations.map(e => `<div class="breakdown-item">${e}</div>`).join('')
...
function wizardCopy() {
  ...
  setTimeout(() => { btn.textContent = 'TITEL KOPIEREN'; }, 2000);
}
